...
async function loadMessages(receiverId) {
  try {
    const msgs = await window.api.getMessages(currentUser.id, receiverId)
...
async function sendMsg(receiverId) {
  ...
  try {
    await window.api.sendMessage(currentUser.id, receiverId, text)
...
async function searchUser() {
  if (!window.api?.searchUsers) {
    document.getElementById("results").innerHTML =
      '<div class="msg error" style="margin:16px">API não carregada.</div>'
    return
  }
  ...
  try {
    const users = await window.api.searchUsers(value)
    results.innerHTML = ''
    ...
    users.forEach(u => {
      ...
      const initials = getInitials(u.username || u.email)
      results.innerHTML += `
...
async function login() {
  ...
  if (user) {
    ...
    msg.innerHTML = `<div class="msg error">Email ou senha inválidos</div>`
  }
}
...
async function confirmDeleteAccount() {
  ...
  try {
    await window.api.deleteUser(currentUser.id, password)
...
async function changePassword() {
  ...
  try {
    await window.api.changePassword(currentUser.id, current, newPwd)
